...
CopyableCanvasLayer::Initialize(const Data& aData)
{
  ...
  if (aData.mSurface) {
    ...
    mDeprecatedSurface =
      new gfxImageSurface(dss->GetData(), // nullcheck?
                          ThebesIntSize(dss->GetSize()),
                          dss->Stride(),
                          SurfaceFormatToImageFormat(dss->GetFormat()));
...
void
CopyableCanvasLayer::UpdateSurface(DrawTarget* aDestTarget,
